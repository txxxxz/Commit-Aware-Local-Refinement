"""
LLaDA sampler adapter: wraps LLaDA-style discrete diffusion mask predictor for
...
def _ensure_torch():
    try:
        ...
        raise ImportError("LLaDA backend requires torch. Install with: pip install torch")
...
def _ensure_transformers():
    try:
        ...
        raise ImportError("LLaDA backend requires transformers. Install with: pip install transformers")


class LLaDASamplerAdapter(SamplerAdapter):
    """
    SamplerAdapter for LLaDA mask-prediction forward.
    ...
    def __init__(
        ...
        if mask_id is not None:
            ...
            # LLaDA default [MASK] id used by upstream generation.py
            self.mask_id = 126336

        vocab = getattr(tokenizer, "vocab", None) or getattr(tokenizer, "get_vocab", lambda: {})()
        self.vocab_size = len(vocab) if vocab else tokenizer.vocab_size
